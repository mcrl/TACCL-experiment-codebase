...
def add_output_file(parser):
    print(f"TCCL: add_output_file() (common.py)")
...
def parse_and_get_topo(node_topology: NodeTopology, comm_sketch_file, reduce=False):
    print("TCCL: parse_and_get_topo() (common.py)")
    ...
    """ TCCL: Intra-node switch based topologies e.g., DGX2 """
    ...
    if copies > 1:
        ...
        """ TCCL: Set inter-node connection. """
        ...
        """ TCCL """
        print(f"TCCL: nnodes: {copies} internode_conn: {internode_conn.items()}")
        ...
        """ TCCL """
        print(f"TCCL: total_outgoing_links: {total_outgoing_links} beta_split_factor: {beta_split_factor}")
        ...
        """ TCCL """
        print(f"TCCL: gpus_to_sender_rev_map: {gpus_to_sender_rev_map}")
