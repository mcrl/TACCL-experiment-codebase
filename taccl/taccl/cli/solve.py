...
def optimize_comm_sketch(topology, route_sketch, collective, distribute_over_links=False):
    print(f"TCCL: optimize_comm_sketch() (solve.py)"
    f"topology: {topology}"
    f"route_sketch: {route_sketch}"
...
def check_heur_comm_sketch(topology, route_sketch, collective, ts_heur):
    print("TCCL: check_heur_comm_sketch() (solve.py)")
...
def make_handle_solve_comm_sketch(cmd_parsers):
    print("TCCL: make_handle_solve_comm_sketch() (solve.py)")

    name = 'solve'
    cmd = cmd_parsers.add_parser(name)

    """ TCCL: Adds topology-related arguments to `cmd_parsers` and inits constructors in KnownTopologies """
    topologies = KnownTopologies(cmd) 
    """ TCCL: Adds collective-related arguments to `cmd_parsers` and inits constructors in KnownCollectives """
    ...
    """
    TCCL: Handler for solve command
    
    """
    def handle(args, command):
        print("TCCL: start handle solve handler (solve.py)")
        ...
        """ TCCL: Create node topologies according to topology file """
        node_topology = topologies.create(args)
        """ TCCL: Create logical topologies from sketches """
...
def make_handle_combine_comm_sketch(cmd_parsers):
    print("TCCL: make_handle_combine_comm_sketch() (solve.py)")
